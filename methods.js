...
function addEmailAdress(emailList, emailAdress) {
  emailList.push(emailAdress);
}

addEmailAdress(emailadresses, 'm.westerbroek@novi.nl');
console.log(emailadresses);
...
function getEmail (emailList, lastName) {
  // Onderstaande map functie werkt niet?
  ...
  for (let i = 0; i < emailList.length; i++) {
    if (emailList[i].includes(lastName)) {
      return emailList[i];
    }
  }
  return false;
...
console.log(getEmail(emailadresses, 'stuivenberg'));
...
function addEmailAtIndex(emailList, emailAdd, emailIndex) {
  emailList.splice(emailList.indexOf(emailIndex), 0, emailAdd);
}

addEmailAtIndex(emailadresses, 't.mellink@novi.nl', 'm.westerbroek@novi.nl');
console.log(emailadresses);
...
function replaceEmail(emailList, emailAdd, emailIndex) {
  emailList.splice(emailList.indexOf(emailIndex), 1, emailAdd);
}

replaceEmail(emailadresses, 'm.vandergeest@novi.nl', 'm.vandergeest@novi-education.nl');
console.log(emailadresses);